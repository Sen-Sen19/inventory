...
$( document ).ready(function() {
    ...
    $('#scanning_manual').modal('show');
    document.getElementById('kanban_no_initial').disabled = true;
    document.getElementById('location_name_initial2').disabled = true;
});
...
const get_section =()=>{
    $.ajax({
        url: 'process/processor_manual.php',
        type: 'POST',
        cache: false,
        data:{
            method: 'get_section'
...
const get_location_name_initial =()=>{
    $.ajax({
        ...
        data:{
            ...
            console.log(data);
            $('#location_name_initial2').html(data);
        }
    });
}

const inventory_type_check = () => {
    var inventory_type = document.getElementById('inventory_type').value;
    if (inventory_type != '') {
        document.getElementById('inventory_type').disabled = true;
        document.getElementById('section_initial2').disabled = false;
        document.getElementById('location_name_initial2').disabled = true;
        suggest_section(inventory_type);
    } else {
        document.getElementById('section_initial2').disabled = true;
        document.getElementById('location_name_initial2').disabled = true;
    }
}

const suggest_section = inventory_type =>{
    let url = '';
    let method = '';

    if (inventory_type == 'Initial') {
        url = 'process/processor_manual.php';
        method = 'get_section';
    } else if (inventory_type == 'Final') {
        url = 'process/processor_manual_final.php';
...
const section_check = () => {
    var section = document.getElementById('section_initial2').value;
    if (section != '') {
        document.getElementById('inventory_type').disabled = true;
        document.getElementById('section_initial2').disabled = true;
        document.getElementById('location_name_initial2').disabled = false;
        suggest_location(section);
    } else {
        document.getElementById('inventory_type').disabled = true;
        document.getElementById('section_initial2').disabled = false;
        document.getElementById('location_name_initial2').disabled = true;
...
const suggest_location = section => {
    var inventory_type = document.getElementById('inventory_type').value;
...
const location_check = () => {
	var location = document.getElementById('location_name_initial2').value;
	if (location != '') {
        document.getElementById('inventory_type').disabled = true;
        document.getElementById('section_initial2').disabled = true;
        document.getElementById('location_name_initial2').disabled = true;
